...
const int voiceDiagnosticsMaximumRecordBytes = 256 * 1024;
const String voiceDiagnosticsEventIdField = '_voiceEventId';

@immutable
final class VoiceDiagnosticsState {
  const VoiceDiagnosticsState({
    this.enabled = false,
    this.captureId,
    this.startedAtUtc,
  ...
  VoiceDiagnosticsState copyWith({
    bool? enabled,
...
@immutable
final class VoiceDiagnosticRecord {
  VoiceDiagnosticRecord({
    ...
    String? homeDirectory,
  }) : writerId = VoiceDiagnosticsRedactor.scrub(
         writerId,
         homeDirectory: homeDirectory,
         maximumLength: 256,
       ),
       timestampUtc = timestampUtc.toUtc(),
       captureId = VoiceDiagnosticsRedactor.scrub(
         captureId,
         homeDirectory: homeDirectory,
         maximumLength: 256,
       ),
       event = VoiceDiagnosticsRedactor.scrub(
         event,
         homeDirectory: homeDirectory,
         maximumLength: 1024,
       ),
       component = VoiceDiagnosticsRedactor.scrub(
         component,
         homeDirectory: homeDirectory,
         maximumLength: 1024,
       ),
       ...
       data = Map.unmodifiable(
         VoiceDiagnosticsRedactor.data(data, homeDirectory: homeDirectory),
       );
  ...
  VoiceDiagnosticRecord copyWith({
    String? writerId,
    int? sequence,
  ...
  static VoiceDiagnosticRecord? fromJson(
    Object? value, {
    String? homeDirectory,
  }) {
...
int voiceDiagnosticSerializedBytes(VoiceDiagnosticRecord record) =>
    utf8.encode(jsonEncode(voiceDiagnosticLine(record))).length + 1;

Map<String, Object?> voiceDiagnosticLine(VoiceDiagnosticRecord record) => {
...
VoiceDiagnosticRecord fitVoiceDiagnosticRecord(
  VoiceDiagnosticRecord record, {
  int maximumBytes = voiceDiagnosticsMaximumRecordBytes,
}) {
  if (voiceDiagnosticSerializedBytes(record) <= maximumBytes) return record;
  ...
  VoiceDiagnosticRecord candidate(String prefix) => record.copyWith(
    message: null,
    data: {
  ...
  while (lower <= upper) {
    ...
    if (voiceDiagnosticSerializedBytes(attempted) <= maximumBytes) {
      best = attempted;
      lower = middle + 1;
    } else {
      upper = middle - 1;
...
abstract final class VoiceDiagnosticsRedactor {

...
const Duration voiceDiagnosticsRetentionAge = Duration(days: 7);
const int voiceDiagnosticsSegmentCount = 5;
const int voiceDiagnosticsSegmentBytes = 10 * 1024 * 1024;
const int voiceDiagnosticsRetentionBytes =
    voiceDiagnosticsSegmentCount * voiceDiagnosticsSegmentBytes;
const int voiceDiagnosticsDecodedTailBytes = 10 * 1024 * 1024;
...
final class VoiceDiagnosticsPersistenceState {
  const VoiceDiagnosticsPersistenceState({
    this.records = const [],
  ...
  final List<VoiceDiagnosticRecord> records;
  ...
  final Map<String, VoiceDiagnosticsActiveCapture> activeCaptures;
}

final class VoiceDiagnosticsActiveCapture {
  const VoiceDiagnosticsActiveCapture({
...
abstract interface class VoiceDiagnosticsPersistence {
  Future<VoiceDiagnosticsPersistenceState> load({required DateTime nowUtc});
  ...
  Future<VoiceDiagnosticsPersistenceState> compact({required DateTime nowUtc});
...
typedef VoiceDiagnosticsReportSinkFactory =
...
/// identities.
Set<String> voiceDiagnosticsEventIdsInJsonReport(
  String report, {
  ...
  while (offset < report.length) {
    ...
    try {
      ...
      final eventId = data[voiceDiagnosticsEventIdField];
...
abstract interface class SnapshotStreamingVoiceDiagnosticsPersistence {
  Future<void> writeJsonReportSnapshotTo({
    required Set<String> candidateEventIds,
    required VoiceDiagnosticsReportSinkFactory outputForRetainedEventIds,
    required DateTime generatedAtUtc,
    required int reportFormatVersion,
    required Map<String, Object?> state,
  });
...
final class MemoryVoiceDiagnosticsPersistence
    implements
        VoiceDiagnosticsPersistence,
        SnapshotStreamingVoiceDiagnosticsPersistence,
        RetainedVoiceDiagnosticsEventIdsPersistence {
  final _VoiceDiagnosticsStore _store = _VoiceDiagnosticsStore();
  ...
  Future<VoiceDiagnosticsPersistenceState> append(
    ...
    for (final record in records) {
      _store.add(fitVoiceDiagnosticRecord(record));
    }
    _store.retain(nowUtc);
    return _store.snapshot;
  }
  ...
  Future<String> buildJsonReport({
    ...
    return _buildJsonReport(
      ...
      state: state,
      segmentCount: voiceDiagnosticsSegmentCount,
      segmentBytes: voiceDiagnosticsSegmentBytes,
    );
  }
  ...
  Future<void> writeJsonReportSnapshotTo({
    ...
    _store.retain(generatedAtUtc);
    final records = List<VoiceDiagnosticRecord>.unmodifiable(_store.records);
    final retainedEventIds = <String>{
      for (final record in records)
        if (record.data[voiceDiagnosticsEventIdField] case final String id
...
typedef VoiceCompactionFaultInjector =
    FutureOr<void> Function(String phase, int index);

final Map<String, _VoiceFileOperationCoordinator>
_voiceFileOperationCoordinators = {};
...
final class FileVoiceDiagnosticsPersistence
    implements
        VoiceDiagnosticsPersistence,
        StreamingVoiceDiagnosticsPersistence,
        SnapshotStreamingVoiceDiagnosticsPersistence,
        RetainedVoiceDiagnosticsEventIdsPersistence {
  FileVoiceDiagnosticsPersistence(
    this.file, {
    this.homeDirectory,
    int segmentCount = voiceDiagnosticsSegmentCount,
    int segmentBytes = voiceDiagnosticsSegmentBytes,
    int decodedTailLimit = 2000,
    int decodedTailBytes = voiceDiagnosticsDecodedTailBytes,
  ...
  static const String directoryName = 'voice-diagnostics';
  static const String fileName = 'voice-diagnostics-v1.jsonl';
  ...
  final VoiceCompactionFaultInjector? _compactionFaultInjector;
  Future<void> _tail = Future<void>.value();
  VoiceDiagnosticsPersistenceState _state =
      const VoiceDiagnosticsPersistenceState();
  bool _loaded = false;
  String _fingerprint = '';
  ...
  static Future<FileVoiceDiagnosticsPersistence> applicationSupport() async {
    final support = await getApplicationSupportDirectory();
    return FileVoiceDiagnosticsPersistence(
  ...
  Future<VoiceDiagnosticsPersistenceState> append(
    ...
    final cutoff = nowUtc.toUtc().subtract(voiceDiagnosticsRetentionAge);
    final maximumRecordBytes = min(
      voiceDiagnosticsMaximumRecordBytes,
      _segmentBytes - _metadataLineBytes,
    );
    final fitted = <VoiceDiagnosticRecord>[];
    ...
    for (final original in records) {
      ...
      record = fitVoiceDiagnosticRecord(
        record,
    ...
    for (final record in fitted) {
      output.writeln(jsonEncode(voiceDiagnosticLine(record)));
    }
  ...
  Future<_VoiceReportSnapshot> _createReportSnapshot({
    required Set<String> candidateEventIds,
    required DateTime generatedAtUtc,
    required int reportFormatVersion,
    required Map<String, Object?> state,
    ...
    try {
      ...
      await for (final decoded in _retainedReportRecords(cutoff)) {
        final eventId = decoded.record.data[voiceDiagnosticsEventIdField];
  ...
  Future<void> _streamReportSnapshot(
    ...
    while (true) {
      final bytes = await input.read(_voiceReportSnapshotReadBytes);
  ...
  Future<void> clear() => _serialize(() async {
    _state = const VoiceDiagnosticsPersistenceState();
  ...
  Future<VoiceDiagnosticsPersistenceState> _loadNow(DateTime nowUtc) async {
    _loaded = true;
  ...
  void _adoptAppended(List<VoiceDiagnosticRecord> records, int encodedBytes) {
    final tail = ListQueue<VoiceDiagnosticRecord>.from(_state.records);
    ...
    final activeCaptures = Map<String, VoiceDiagnosticsActiveCapture>.of(
    ...
    _state = VoiceDiagnosticsPersistenceState(
  ...
  Future<_FileScan> _scan(DateTime nowUtc) async {
    ...
    var activeCaptures = <String, VoiceDiagnosticsActiveCapture>{};
    ...
    return _FileScan(
      state: VoiceDiagnosticsPersistenceState(
  ...
  Future<void> _compactNow(
    DateTime nowUtc, {
    List<VoiceDiagnosticRecord> additionalRecords = const [],
  ...
  static List<int> _paddedMetadataLine(_FileMetadata metadata) {
    ...
    if (encoded.length >= _metadataLineBytes) {
      throw StateError('Voice diagnostics metadata exceeds its reservation.');
    }
    return [
      ...encoded,
  ...
  Future<T> _serialize<T>(Future<T> Function() operation) {
    ...
    _tail = _tail.then((_) async {
      try {
        final coordinator = _voiceFileOperationCoordinators.putIfAbsent(
          _lockFile.absolute.path,
          () => _VoiceFileOperationCoordinator(_lockFile),
        );
...
final class _VoiceFileOperationCoordinator {
  _VoiceFileOperationCoordinator(this.lockFile);
...
final class _FileScan {
  ...
  final VoiceDiagnosticsPersistenceState state;
  final DateTime? oldestTimestampUtc;
...
final class _TemporaryGroup {
  _TemporaryGroup(this.file)
    : bytes = FileVoiceDiagnosticsPersistence._metadataLineBytes;
...
String _randomReportSnapshotSuffix() => List<int>.generate(
  16,
  (_) => _voiceReportSnapshotRandom.nextInt(256),
...
bool _acceptIdentity(VoiceDiagnosticRecord record, Map<String, int> highWater) {
...
void _rememberActiveCapture(
  ...
  while (captures.length > _voiceDiagnosticsActiveCaptureLimit) {
...
_FileMetadata? _decodeStateLine(String line) {
  try {
    ...
    final activeCaptures = <String, VoiceDiagnosticsActiveCapture>{};
...
final class _VoiceDiagnosticsStore {
  List<VoiceDiagnosticRecord> _records = [];
  ...
  List<VoiceDiagnosticRecord> get records =>
      UnmodifiableListView<VoiceDiagnosticRecord>(_records);
  ...
  VoiceDiagnosticsPersistenceState get snapshot =>
      VoiceDiagnosticsPersistenceState(
        records: records,
  ...
  bool add(VoiceDiagnosticRecord record) {
    ...
    if (duplicate) {
      ...
      retainedBytes -= voiceDiagnosticSerializedBytes(_records[duplicateIndex]);
  ...
  void retain(
    DateTime nowUtc, {
    int maximumBytes = voiceDiagnosticsRetentionBytes,
  }) {
    final cutoff = nowUtc.toUtc().subtract(voiceDiagnosticsRetentionAge);
    if (!_timestampsMonotonic) {
      final fresh = <VoiceDiagnosticRecord>[];
      var freshBytes = 0;
      for (final record in _records) {
        if (!record.timestampUtc.isAfter(cutoff)) {
          noteDropped(1);
        ...
        fresh.add(record);
        freshBytes += voiceDiagnosticSerializedBytes(record);
      }
      _records = fresh;
    ...
    while (removeCount < _records.length &&
      ...
      retainedBytes -= voiceDiagnosticSerializedBytes(_records[removeCount]);
      removeCount += 1;
    }
  ...
  static bool _isTimestampMonotonic(List<VoiceDiagnosticRecord> records) {
...
void _writeJsonReportRecordsTo(
  StringSink output,
  Iterable<VoiceDiagnosticRecord> records, {
  required DateTime generatedAtUtc,
  required int reportFormatVersion,
  required Map<String, Object?> state,
  required int segmentCount,
...
Map<String, Object?> _reportHeader({
  ...
  'captureFormatVersion': voiceDiagnosticsFormatVersion,
  'record': 'report',
  'generatedAtUtc': generatedAtUtc.toUtc().toIso8601String(),
  'app': {'version': AppRelease.version},
  'platform': {
    'operatingSystem': Platform.operatingSystem,
    'operatingSystemVersion': VoiceDiagnosticsRedactor.scrub(
      Platform.operatingSystemVersion,
    ),
  },
  'retention': {
    'days': voiceDiagnosticsRetentionAge.inDays,
    'segmentCount': segmentCount,
    'segmentBytes': segmentBytes,
    'maximumRecordBytes': voiceDiagnosticsMaximumRecordBytes,
  },
  'state': state,
};
...
Stream<String?> _boundedJsonLines(File file) async* {
  ...
  void append(List<int> chunk, int start, int end) {
    ...
    if (bytes.length + end - start >= voiceDiagnosticsMaximumRecordBytes) {
      bytes.clear();
      discarding = true;
      return;
    }

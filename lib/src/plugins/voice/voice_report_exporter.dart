...
enum VoiceReportExportOutcome { shared, saved, cancelled }

abstract interface class VoiceReportExporter {
...
typedef VoiceReportWriter = FutureOr<void> Function(StringSink output);

final class VoiceClipboardReport {
  const VoiceClipboardReport(this.text, {required this.truncated});

  final String text;
  final bool truncated;
}

VoiceClipboardReport boundVoiceReportForClipboard(
  String report, {
  required int byteLimit,
}) {
  if (utf8.encode(report).length <= byteLimit) {
    return VoiceClipboardReport(report, truncated: false);
  }
...
abstract interface class StreamingVoiceReportExporter {
  Future<VoiceReportExportOutcome> exportGenerated(
    VoiceReportWriter writer, {
    Rect? sharePositionOrigin,
  });
}

abstract interface class VoiceReportExportEnvironment {
...
final class NativeVoiceReportExporter
    implements VoiceReportExporter, StreamingVoiceReportExporter {
  NativeVoiceReportExporter({
    TargetPlatform? platform,
    VoiceReportExportEnvironment? environment,
  ...
  final VoiceReportExportEnvironment _environment;
  ...
  Future<VoiceReportExportOutcome> exportGenerated(
    ...
    if (_platform == TargetPlatform.linux) {
      ...
      if (destination == null) return VoiceReportExportOutcome.cancelled;
  ...
  static String _filename(DateTime timestampUtc) {
    final stamp = timestampUtc
        ...
        .replaceAll(':', '-')
        .replaceAll('.', '-');
    return 'voice-diagnostics-$stamp.jsonl';
  }
...
final class _NativeExportEnvironment implements VoiceReportExportEnvironment {
  ...
  Future<VoiceReportExportOutcome> shareReport(
    ...
    return result.status == sharing.ShareResultStatus.dismissed
        ? VoiceReportExportOutcome.cancelled
        : VoiceReportExportOutcome.shared;
  }
}

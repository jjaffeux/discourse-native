...
import 'voice_diagnostics_persistence.dart';

export 'package:discourse_native/discourse_plugin_sdk.dart'
    show DiagnosticSeverity;

export 'voice_diagnostics_models.dart';
export 'voice_diagnostics_persistence.dart';

final Set<String> _liveVoiceDiagnosticsWriterIds = {};

typedef VoiceDiagnosticsTimerFactory =
    Timer Function(Duration duration, void Function() callback);

abstract interface class VoiceDiagnosticsRecorder {
...
abstract interface class VoiceDiagnosticsFlusher {
  Future<void> flushDiagnostics();
}

final class NoopVoiceDiagnosticsRecorder implements VoiceDiagnosticsRecorder {
...
/// active between [VoiceDiagnosticsController.startCapture] and
/// [VoiceDiagnosticsController.stopCapture]. A bridge should feed vendor
/// lines to [VoiceDiagnosticsRecorder.recordRaw].
abstract interface class VoiceDiagnosticsSdkLogBridge {
  FutureOr<void> install(VoiceDiagnosticsRecorder recorder);

  FutureOr<void> uninstall();
}

typedef VoiceDiagnosticsSdkLogInstaller =
    FutureOr<void> Function(VoiceDiagnosticsRecorder recorder);
typedef VoiceDiagnosticsSdkLogUninstaller = FutureOr<void> Function();

final class CallbackVoiceDiagnosticsSdkLogBridge
    implements VoiceDiagnosticsSdkLogBridge {
  const CallbackVoiceDiagnosticsSdkLogBridge({
    required VoiceDiagnosticsSdkLogInstaller install,
    required VoiceDiagnosticsSdkLogUninstaller uninstall,
  }) : _install = install,
       _uninstall = uninstall;

  final VoiceDiagnosticsSdkLogInstaller _install;
  final VoiceDiagnosticsSdkLogUninstaller _uninstall;
...
final class VoiceDiagnosticsController implements VoiceDiagnosticsRecorder {
  ...
  final ListQueue<VoiceDiagnosticRecord> _records;
  final ListQueue<VoiceDiagnosticRecord> _pendingWrites = ListQueue();
  final List<VoiceDiagnosticsSdkLogBridge> _installedSdkLogBridges = [];
  ...
  static Future<VoiceDiagnosticsController> create({
    ...
    final writerId = VoiceDiagnosticsRedactor.scrub(
  ...
  ValueListenable<List<VoiceDiagnosticRecord>> get eventsListenable =>
      _eventsNotifier;

  ValueListenable<bool> get captureEnabledListenable => _captureEnabledNotifier;

  VoiceDiagnosticsState get state => _state;

  List<VoiceDiagnosticRecord> get events => _eventsNotifier.value;

  List<VoiceDiagnosticRecord> get eventsTail => events;
  ...
  Future<void> _startCapture() async {
    ...
    final captureId = VoiceDiagnosticsRedactor.scrub(
      _captureIdFactory(),
    ...
    _recordCaptured(
      ...
      correlationId: null,
      message: null,
      data: {'formatVersion': voiceDiagnosticsFormatVersion},
      captureId: captureId,
  ...
  void record(
    ...
    if (_closed) return;
    final safeEvent = VoiceDiagnosticsRedactor.scrub(
      event,
      homeDirectory: _homeDirectory,
      maximumLength: 1024,
    );
    final safeComponent = VoiceDiagnosticsRedactor.scrub(
      component,
      homeDirectory: _homeDirectory,
      maximumLength: 1024,
    );
    ...
    final safeData = VoiceDiagnosticsRedactor.data(
      data,
      homeDirectory: _homeDirectory,
    );
    final eventId = _reporter.newCorrelationId('voice-event');
    final attributedData = <String, Object?>{
      ...safeData,
      voiceDiagnosticsEventIdField: eventId,
    };
    try {
      _reporter.recordLog(
        name: safeEvent,
  ...
  void recordRaw(
    ...
    final rawData = Map<String, Object?>.of(data)
      ..remove(voiceDiagnosticsEventIdField);
    _recordCaptured(
      event: event,
      component: component,
      severity: severity,
  ...
  Future<void> _clear() async {
    ...
    if (_durableStorageUnavailable) {
      final error = StateError(
        'Durable Voice diagnostics storage is unavailable; deletion cannot '
        'be confirmed.',
      );
      _reportPersistenceError(error, operation: 'voice.deep_capture.clear');
      throw error;
    }
  ...
  Future<void> writeJsonReportTo(StringSink output) async {
    ...
    if (persistence is StreamingVoiceDiagnosticsPersistence) {
      final streaming = persistence as StreamingVoiceDiagnosticsPersistence;
  ...
  Future<void> writeJsonReportSnapshotTo({
    ...
    if (persistence is SnapshotStreamingVoiceDiagnosticsPersistence) {
      final streaming =
          persistence as SnapshotStreamingVoiceDiagnosticsPersistence;
    ...
    final retainedEventIds = voiceDiagnosticsEventIdsInJsonReport(
      report,
  ...
  Future<Set<String>> findRetainedEventIds(
    ...
    _publishUiNow();
    final persistence = _persistence;
    if (persistence is! RetainedVoiceDiagnosticsEventIdsPersistence) {
      throw UnsupportedError(
        '${persistence.runtimeType} cannot scan retained event identifiers.',
      );
    }
    return (persistence as RetainedVoiceDiagnosticsEventIdsPersistence)
  ...
  Future<void> _close() async {
    ...
    try {
      ...
      _reportPersistenceError(error, operation: 'voice.deep_capture.close');
    }
    ...
    _liveVoiceDiagnosticsWriterIds.remove(_writerId);
  }
  ...
  void _recordCaptured({
    ...
    final recordBytes = voiceDiagnosticSerializedBytes(record);
    ...
    if (controlRecord) {
      ...
      while (_pendingWritesBytes + recordBytes > controlLimit) {
        ...
        final disposableBytes = voiceDiagnosticSerializedBytes(disposable);
    ...
    while (_records.length > _eventsTailLimit ||
      ...
      _eventsTailBytes -= voiceDiagnosticSerializedBytes(
  ...
  void _scheduleRetentionExpiry({Duration? retryDelay}) {
    ...
    final untilExpiry = oldest
        .add(voiceDiagnosticsRetentionAge)
        .difference(nowUtc);
    final delay =
  ...
  void _dropFailedBatch(List<VoiceDiagnosticRecord> batch) {
  ...
  void _markPersistenceFailure(
    ...
    _reportPersistenceError(error, operation: 'voice.deep_capture.$phase');
  }

  void _reportPersistenceError(Object error, {required String operation}) {
    try {
      _reporter.reportError(
        _SafeVoiceDiagnosticsFailure(
          operation: operation,
  ...
  void _recordBridgeFailure(String phase, Object error, StackTrace stackTrace) {
    ...
    _reportPersistenceError(
      error,
      operation: 'voice.deep_capture.sdk_bridge.$phase',
    );
  }
}

final class _SafeVoiceDiagnosticsFailure implements Exception {
  const _SafeVoiceDiagnosticsFailure({
  ...
  @override
  String toString() => 'Voice diagnostics failure ($operation; $originalType)';
}
...
String _newCaptureId() {
  ...
  try {
    ...
    return 'voice-$now-${high.toRadixString(16).padLeft(8, '0')}'
...
String _newWriterId() => _newCaptureId().replaceFirst('voice-', 'writer-$pid-');

bool _isVoiceDiagnosticsWriterAlive(String writerId) {
  if (_liveVoiceDiagnosticsWriterIds.contains(writerId)) return true;
...
bool _isCaptureControlRecord(VoiceDiagnosticRecord record) =>
    switch (record.event) {

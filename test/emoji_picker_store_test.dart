...
void main() {
  ...
  group('loading and forum persistence', () {
    test(
      'round-trips canonical URLs through SharedPreferences per forum',
      () async {
        ...
        final otherForumStore = EmojiPickerStore();
        const otherForum = 'https://team.discourse.org';
        expect(
          await otherForumStore.readSkinTone(siteUrl: otherForum),
          EmojiSkinTone.neutral,
        );
        expect(
          await otherForumStore.favoriteEmojiCodes(
            siteUrl: otherForum,
            context: CoreEmojiUsageContexts.topic,
            catalog: _catalog(['wave']),
          ),
    ...
    test(
      'preserves an unread document and resumes writes after recovery',
      () async {
        ...
        expect(
          reloaded.favoriteEmojiCodesFor(
            siteUrl: meta,
            context: CoreEmojiUsageContexts.topic,
            catalog: _catalog(['wave', 'heart', 'smile']),
          ),
          ['smile', 'heart', 'wave'],
        );
      },
  ...
  group('usage history', () {
    test('ranks favorites by frequency and then recency', () async {
    ...
    test('retains the 40 most recent events', () async {
      ...
      expect(topic, [
        for (var index = 6; index < 45; index++) 'emoji_$index',
        'removed_from_site:t4',
      ]);
    });

    test('returns at most 20 favorites that remain in the catalog', () async {
      final store = EmojiPickerStore(persistence: _MemoryPersistence());
      ...
      final favorites = await store.favoriteEmojiCodes(
        ...
        context: CoreEmojiUsageContexts.topic,
        catalog: _catalog([
          for (var index = 0; index < 21; index++) 'emoji_$index',
        ]),
      );
      expect(favorites, [
        for (var index = 20; index > 0; index--) 'emoji_$index',
      ]);
    });

    test(
      'clears only the requested context and preserves the forum tone',
      () async {
        final store = EmojiPickerStore(persistence: _MemoryPersistence());
    ...
    test('preserves explicit t2 through t6 codes across persistence', () async {
      final persistence = _MemoryPersistence();
      final store = EmojiPickerStore(persistence: persistence);
  ...
  group('context migration and isolation', () {
    test(
      'adopts postReactions for reactions while upgrading a v1 document',
      () async {
        final persistence = _MemoryPersistence()
        ...
        expect(upgradedHistory, {
          'topic': ['wave'],
          'chat': ['heart:t2'],
          reactionsEmojiUsageContext.id: ['clap', 'tada'],
        });
    ...
    test('adopts a legacy key when its namespaced context changes', () async {
      final persistence = _MemoryPersistence()
        ..values[meta] = jsonEncode({
          'version': 1,
          'tone': 'neutral',
      ...
      expect(upgraded['version'], EmojiPickerStore.formatVersion);
      expect(history, {
        chatEmojiUsageContext.id: ['heart', 'wave'],
      });
    });

    test('namespaces same-named plugin contexts by owner', () async {
      ...
      expect(history, {
        firstContext.id: ['wave'],
        secondContext.id: ['heart'],
      });
    });

    test('preserves histories owned by unavailable plugins', () async {
      const activeContext = EmojiUsageContext(
        owner: PluginId('active-plugin'),
        name: 'composer',
      );
      const unavailableContextId = 'removed-plugin/message';
      final persistence = _MemoryPersistence()
        ..values[meta] = jsonEncode({
          'version': EmojiPickerStore.formatVersion,
          'tone': 'neutral',
          'history': {
            unavailableContextId: ['tada'],
          },
        });
      final store = EmojiPickerStore(persistence: persistence);
      ...
      expect(history, {
        unavailableContextId: ['tada'],
        activeContext.id: ['wave'],
      });
    });
  });

  group('storage failure handling', () {
    test('falls back safely and reports a corrupt read', () async {
      final persistence = _MemoryPersistence()..values[meta] = '{not json';
      final diagnostics = await _installDiagnostics(
        'emoji-picker-corrupt-read',
      );
      final store = EmojiPickerStore(persistence: persistence);
    ...
    test(
      'reports a rejected write while retaining the in-memory mutation',
      () async {
  ...
  group('write ordering', () {
    ...
    test(
      'serializes replacement stores through their shared persistence',
      () async {
        final gate = Completer<void>();
        final persistence = _ControlledPersistence(firstWriteGate: gate);

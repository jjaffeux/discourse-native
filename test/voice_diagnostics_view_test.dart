...
void main() {
  ...
  testWidgets('shows metadata and searches latest-first capture details', (
    ...
    final harness = _Harness(
      state: const VoiceDiagnosticsUiState(
        enabled: false,
    ...
    expect(
      find.byKey(const ValueKey('voice-truncated-indicator')),
      findsOneWidget,
    );
    expect(
    ...
    await tester.tap(
      find.byKey(const ValueKey('voice-diagnostics-detail-back')),
    );
    await tester.pump();
    expect(
      find.byKey(const ValueKey('voice-diagnostics-search')),
  ...
  testWidgets('delegates clipboard and streaming export generation', (
    ...
    expect(find.text('Voice report saved'), findsOneWidget);
  });
}
...
final class _Harness {
  _Harness({
    VoiceDiagnosticsUiState state = const VoiceDiagnosticsUiState(
      enabled: false,
      retainedBytes: 0,
      droppedRecords: 0,
      truncated: false,
    ...
    this.events = const [],
    this.report = '{"kind":"voice_report"}',
  }) : state = ValueNotifier(state);

  final ValueNotifier<VoiceDiagnosticsUiState> state;
  ...
  Future<void> startCapture() async {
    startCount += 1;
    state.value = VoiceDiagnosticsUiState(
      enabled: true,
      captureId: 'capture-1',
  ...
  Future<VoiceClipboardReport> buildClipboardReport(int byteLimit) async {
    clipboardBuildCount += 1;
    return const VoiceClipboardReport(

...
void main() {
  group('reading a message', () {
    ...
    test('is edited only when the site said so, which it does by adding the key', () {
    ...
    test('remembers what a reply was replying to, because the chaining depends on it', () {
      final read = messageFrom(
        message(
          inReplyTo: const {
            'id': 7,
            'cooked': '<p>before</p>',
            'excerpt': 'before',
            'user': {'id': 3, 'username': 'kris', 'avatar_template': '/k/{size}.png'},
          },
        ),
      );
    ...
    test('reads reactions only when there are some, the key being dropped otherwise', () {
    ...
    test('sorts an upload by its filename when the site named no extension', () {
      final read = messageFrom(
        message(
          uploads: const [
    ...
    test('reads nothing it was not sent rather than inventing defaults it was', () {
  ...
  group('a page of messages', () {
    ...
    test('reads an unanswered can_load_more_past as no more rather than as yes', () {
      ...
      expect(
        ChatMessage.parsePage(page([], canLoadMorePast: null), site)
            .canLoadMorePast,
        isFalse,
      );
      expect(
        ChatMessage.parsePage(page([], canLoadMorePast: true), site)
            .canLoadMorePast,
        isTrue,
      );
      expect(
        ChatMessage.parsePage(page([], canLoadMorePast: false), site)
            .canLoadMorePast,
        isFalse,
      );
    });

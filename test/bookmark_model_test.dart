...
void main() {
  test('activity bookmarks retain core and plugin target metadata', () {
  ...
  test('absent and malformed activity metadata stays readable', () {
    final bookmark = Bookmark.fromJson(const {
      'id': 'not-an-id',
  ...
  test('bookmark copying and value equality include mutable state', () {
  ...
  test('post bookmark metadata is complete and malformed state is total', () {
  ...
  test('topic details derive topic and sorted post bookmarks', () {
    ...
    expect(payload.detail.postBookmarks.map((bookmark) => bookmark.id), [2, 3]);
    expect(payload.detail.hasBookmarks, isTrue);
    expect(payload.detail.withoutBookmarks().hasBookmarks, isFalse);
  });

  test('topic, user and site settings preserve bookmark preferences', () {
    ...
    expect(
      SiteConfig.fromJson(config.toJson()).suggestWeekendsInDatePickers,
      isFalse,
    );
    expect(

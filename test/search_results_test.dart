...
void main() {
  ...
  group('excerpt parsing', () {
    test(
      'collapses cooked whitespace while retaining highlight boundaries',
      () {
  ...
  group('header-search facets', () {
    test('builds the same ordered result facets as core', () {
      final results = SearchResults.fromJson(const {
        'topics': [
      ...
      expect(
        (results.results[3] as SearchUserHit).avatarUrl,
        contains('/sam/'),
      );
    ...
    test(
      'drops malformed individual facets without dropping valid sections',
      () {
        final results = SearchResults.fromJson(const {
          'categories': [
            {'name': 'No id'},
    ...
    test('bounds every section before parsing its entries', () {

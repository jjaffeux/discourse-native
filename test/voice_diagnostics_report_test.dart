...
import 'package:discourse_native/src/plugins/voice/voice_diagnostics_report.dart';
...
void main() {
  ...
  test('reuses timeline projections when the retained tail advances', () async {
    ...
    final diagnostics = await DiagnosticsController.create(
      ...
      sessionId: 'voice-report-projection',
      clock: () => now,
    );
    final voice = await VoiceDiagnosticsController.create(
    ...
    expect(first, hasLength(VoiceDiagnosticsController.defaultEventsTailLimit));
  ...
  test(
    ...
    () async {
      ...
      final diagnostics = await DiagnosticsController.create(
        persistence: MemoryDiagnosticsPersistence(),
        sessionId: 'voice-report-test',
        clock: () => now,
      );
      final persistence = _TrackingVoiceDiagnosticsPersistence();
      final voice = await VoiceDiagnosticsController.create(
      ...
      voice.record(
        ...
        correlationId: 'voice-call-report',
      );
      now = now.add(const Duration(seconds: 1));
      voice.recordRaw(
        'peer.ice.failed',
      ...
      DiagnosticsSink.runOperation('voice.join', () {
        ...
        diagnostics.recordHttp(
          HttpDiagnosticRecord(
            eventId: 'voice-http-failed',
            phase: HttpDiagnosticPhase.started,
            timestamp: now.add(const Duration(milliseconds: 200)),
            method: 'POST',
            uri: Uri.parse(
              'https://198.51.100.77/voice/rooms/42/signal?token=private',
            ),
            sentBytes: 80,
            receivedBytes: 0,
          ),
      ...
      expect(
        timeline.any(
          (event) => event['event'] == 'POST /voice/rooms/42/join?token',
        ),
        isTrue,
      );

      final retainedBytes = voice.state.retainedBytes;
      ...
      expect(materialized, contains('/voice/rooms/42/join?token'));
      expect(materialized, contains('/voice/rooms/42/signal?token'));
  ...
  test(
    ...
    () async {
      ...
      final diagnostics = await DiagnosticsController.create(
        persistence: MemoryDiagnosticsPersistence(),
        sessionId: 'voice-report-nested-event-id',
        clock: () => now,
      );
      final voice = await VoiceDiagnosticsController.create(
        reporter: PluginDiagnosticsReporter.fixed(diagnostics),
      ...
      voice.recordRaw(
        ...
        component: 'sdk',
        data: {
          'nested': {voiceDiagnosticsEventIdField: ordinaryId},
        },
      );
      now = now.add(const Duration(seconds: 1));
      await voice.stopCapture();
  ...
  test(
    ...
    () async {
      ...
      final diagnostics = await DiagnosticsController.create(
        persistence: MemoryDiagnosticsPersistence(),
        sessionId: 'voice-report-fallback',
        clock: () => now,
      );
      ...
      await voice.startCapture();
      now = now.add(const Duration(seconds: 1));
      voice.record('fallback.captured.once', component: 'controller');
      now = now.add(const Duration(seconds: 1));
      await voice.stopCapture();
      persistence.clearAfterNextBuild = true;

      final output = StringBuffer();
      await VoiceDiagnosticsReport(
        diagnostics: PluginDiagnosticsReadExportHost(diagnostics),
        voice: voice,
      ).writeJsonTo(output);
  ...
  test('bounds materialized clipboard reports by UTF-8 whole lines', () {
    final old = 'old-${'é' * 200}';
    final newest = 'new-${'🙂' * 10}';
    final bounded = boundVoiceReportForClipboard(
      '$old\n$newest',
      byteLimit: 260,
    );
...
final class _TrackingVoiceDiagnosticsPersistence
    implements
...
final class _ClearingAfterBuildPersistence
    implements VoiceDiagnosticsPersistence {
  final MemoryVoiceDiagnosticsPersistence _delegate =
      MemoryVoiceDiagnosticsPersistence();
    ...
    if (jsonDecode(line) case final Map<String, dynamic> record)
      if (record['event'] case final Map<String, dynamic> event)
        if (event['data'] case final Map<String, dynamic> data)
          if (data[voiceDiagnosticsEventIdField] case final String id

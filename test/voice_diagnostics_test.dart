...
void main() {
  ...
  group('diagnostic capture', () {
    test(
      ...
      () async {
        ...
        expect(ordinaryLog.source, 'voice');
        ...
        expect(
          connectedDeep.data[voiceDiagnosticsEventIdField],
          connectedOrdinary.attributes[voiceDiagnosticsEventIdField],
        );
        expect(
          deep.events.singleWhere((event) => event.event == 'sdk.on').data,
          isNot(contains(voiceDiagnosticsEventIdField)),
        );
      },
    );
  ...
  group('diagnostics persistence', () {
    test('caps each JSONL record at 256 KiB and marks truncation', () async {
      ...
      expect(
        voiceDiagnosticSerializedBytes(large),
        lessThanOrEqualTo(voiceDiagnosticsMaximumRecordBytes),
      );
      ...
      expect(
        ...
        everyElement(lessThanOrEqualTo(voiceDiagnosticsMaximumRecordBytes)),
      );
    });

    test('repairs a corrupt tail and marks the interrupted capture', () async {
      final directory = await Directory.systemTemp.createTemp(
        'voice-diagnostics-interrupted-',
      );
      addTearDown(() async {
        if (await directory.exists()) await directory.delete(recursive: true);
      });
      final file = File('${directory.path}/nested/voice.jsonl');
      ...
      final recovered = await VoiceDiagnosticsController.create(
        persistence: FileVoiceDiagnosticsPersistence(file),
    ...
    test(
      ...
      () async {
        final directory = await Directory.systemTemp.createTemp(
          'voice-diagnostics-legacy-redaction-',
        );
        addTearDown(() async {
          if (await directory.exists()) await directory.delete(recursive: true);
        });
        ...
        final malformed =
            '${jsonEncode(voiceDiagnosticLine(_record(2, now, event: 'malformed.regex-match')))} trailing-junk';
        final healthy = jsonEncode(
          voiceDiagnosticLine(_record(3, now, event: 'healthy.after-legacy')),
        );
    ...
    test(
      ...
      () async {
        ...
        final recovered = await FileVoiceDiagnosticsPersistence(
          file,
          segmentCount: 2,
          segmentBytes: 4096,
        ).load(nowUtc: now);
    ...
    test(
      ...
      () async {
        ...
        final reloaded = await FileVoiceDiagnosticsPersistence(
          file,
        ).load(nowUtc: now);
        ...
        final report = await FileVoiceDiagnosticsPersistence(file)
            .buildJsonReport(
              generatedAtUtc: now,
              reportFormatVersion: 1,
              state: const {},
    ...
    test('finds retained deep event IDs outside the decoded tail', () async {
      ...
      final seed = FileVoiceDiagnosticsPersistence(file, decodedTailLimit: 1);
      await seed.append([
        for (var sequence = 1; sequence <= 10; sequence += 1)
          _record(
            ...
            data: {voiceDiagnosticsEventIdField: 'ordinary-$sequence'},
          ),
      ], nowUtc: now);
      final controller = await VoiceDiagnosticsController.create(
        persistence: FileVoiceDiagnosticsPersistence(file, decodedTailLimit: 1),
        clock: () => now,
      );
      addTearDown(controller.close);
      ...
      await expectLater(
        controller.findRetainedEventIds(
          List.generate(
            VoiceDiagnosticsController.maximumRetainedEventIdCandidates + 1,
    ...
    test(
      ...
      () async {
        final directory = await Directory.systemTemp.createTemp(
          'voice-diagnostics-report-snapshot-',
        );
        addTearDown(() async {
          if (await directory.exists()) await directory.delete(recursive: true);
        });
        ...
        final first = FileVoiceDiagnosticsPersistence(file);
        await first.append([
          _record(
            1,
            now,
            ...
            data: {voiceDiagnosticsEventIdField: 'ordinary-snapshot-event'},
          ),
        ], nowUtc: now);
    ...
    test(
      ...
      () async {
        final directory = await Directory(
          '.dart_tool',
        ).createTemp('voice-diagnostics-report-cleanup-');
        addTearDown(() async {
          if (await directory.exists()) await directory.delete(recursive: true);
        });
        final file = File('${directory.path}/voice.jsonl');
        ...
        await persistence.append([
          _record(
            ...
            data: {voiceDiagnosticsEventIdField: 'ordinary-cleanup-event'},
          ),
        ], nowUtc: now);
        final stale = File(
    ...
    test(
      ...
      () async {
        final directory = await Directory.systemTemp.createTemp(
          'voice-diagnostics-multiprocess-',
        );
        addTearDown(() async {
          if (await directory.exists()) await directory.delete(recursive: true);
        });
        ...
        final first = FileVoiceDiagnosticsPersistence(
          file,
          segmentCount: 3,
          segmentBytes: 8192,
        );
        final second = FileVoiceDiagnosticsPersistence(
          file,
          segmentCount: 3,
          segmentBytes: 8192,
        );
    ...
    test(
      ...
      () async {
        final directory = await Directory.systemTemp.createTemp(
          'voice-diagnostics-multiwriter-order-',
        );
        addTearDown(() async {
          if (await directory.exists()) await directory.delete(recursive: true);
        });
        ...
        final first = FileVoiceDiagnosticsPersistence(file);
        final second = FileVoiceDiagnosticsPersistence(file);
        await first.append([
    ...
    test(
      ...
      () async {
        final directory = await Directory.systemTemp.createTemp(
          'voice-diagnostics-active-writers-',
        );
        addTearDown(() async {
          if (await directory.exists()) await directory.delete(recursive: true);
        });
    ...
    test('bounds outstanding capture metadata by writer', () async {
      ...
      final persistence = MemoryVoiceDiagnosticsPersistence();
      final stored = await persistence.append([
    ...
    test('recovers compaction without exposing mixed generations', () async {
      for (var failAt = 0; failAt < 7; failAt += 1) {
        final directory = await Directory.systemTemp.createTemp(
          'voice-diagnostics-generation-$failAt-',
        );
        try {
          final file = File('${directory.path}/voice.jsonl');
  ...
  group('SDK bridge lifecycle', () {
    test('installs the bridge only for the explicit capture window', () async {
      ...
      VoiceDiagnosticsRecorder? installedRecorder;
      var installs = 0;
      var uninstalls = 0;
      final bridge = CallbackVoiceDiagnosticsSdkLogBridge(
        install: (recorder) {
          installs += 1;
    ...
    test('unwinds a bridge that throws after a partial install', () async {
      ...
      final healthy = CallbackVoiceDiagnosticsSdkLogBridge(
        install: (_) => healthyActive = true,
        uninstall: () => healthyActive = false,
      );
      final partial = CallbackVoiceDiagnosticsSdkLogBridge(
        install: (_) {
          partialActive = true;
  ...
  group('the diagnostics controller', () {
    ...
    test(
      ...
      () async {
        ...
        final reopened = await VoiceDiagnosticsController.create(
          persistence: persistence,
          clock: () => now.add(const Duration(minutes: 1)),
  ...
  group('controller failure handling', () {
    test(
      ...
      () async {
        ...
        expect(general, contains('voice.deep_capture.create'));
        expect(general, contains('voice.deep_capture.clear'));
    ...
    test(
      ...
      () async {
        ...
        expect(controller.events, isEmpty);
        expect(general, isNot(contains(secret)));
        expect(general, contains('voice.deep_capture.load'));
        expect(general, contains('_SentinelFailure'));
      },
    );
    ...
    test(
      ...
      () async {
        ...
        final general = ordinary.buildJsonReport();
        expect(general, isNot(contains(secret)));
        expect(general, contains('voice.deep_capture.append'));
        expect(general, contains('_SentinelFailure'));
        persistence.appendError = null;
        await controller.stopCapture();
      },
    ...
    test(
      ...
      () async {
        ...
        expect(general, isNot(contains(secret)));
        expect(general, contains('voice.deep_capture.sdk_bridge.install'));
        expect(general, contains('_SentinelFailure'));
  ...
  group('retention and expiry', () {
    ...
    test(
      ...
      () async {
        ...
        expect(header['captureFormatVersion'], voiceDiagnosticsFormatVersion);
...
Future<void> _mutateStoreWhileSnapshotOutputIsHeld({
  ...
  final persistence = FileVoiceDiagnosticsPersistence(File(storePath));
...
final class _CountingPersistence implements VoiceDiagnosticsPersistence {

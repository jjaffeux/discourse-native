...
void main() {
  group('manifest installation and dependencies', () {
    test(
    ...
    test('orders providers first without reordering other peers', () {
      final installed = PluginInstaller.install(
        const PluginManifest([
          _Module(
            'consumer',
  ...
  group('declared capability claims', () {
    test(
      'declared route, syntax, and exclusive claims cannot be ambiguous',
      () {
    ...
    test(
      'declared claims cannot be omitted or supplemented at registration',
      () {
        for (final mismatch in <PluginModule>[
          const _Module(
            'route-omitted',
  ...
  group('static contributions and key ownership', () {
    test(
      ...
      final installed = PluginInstaller.install(
        PluginManifest([
          _StaticModule(
            'contributor',
            targets: const [
              PluginStaticContributionTarget(
                PluginId('absent'),
                optional: true,
              ),
            ],
    ...
    test(
      'singleton static contribution cardinality is validated at install',
      () {
        const point = PluginStaticContributionPoint<String>(
          owner: PluginId('owner'),
          name: 'singleton',
            ...
            PluginManifest([
              ...
              for (final id in ['first', 'second'])
                _StaticModule(
                  ...
                  registerStatic: (registrar) => registrar
                      .addStaticContribution(point, name: 'item', value: id),
                ),
            ]),
          ),
          throwsA(isA<PluginInstallationException>()),
    ...
    test('reads present and absent singleton contributions', () {
  ...
  group('application lifecycle', () {
              ...
              _Module(
                ...
                appLifecycle: _AppLifecycle(
                  ...
                  inspectBindings: (bindings) {
                    received.add(
                      '$id:${bindings.require(_scopedHostValuePort)}',
                    );
                    received.add(
    ...
    test(
      ...
      () async {
        ...
        final observing = installed.observeAppState(
          'paused',
          foreground: false,
        );
  ...
  group('session authority and lifecycle', () {
    test(
      ...
      () async {
        ...
        expect(
          () => owned.maybe(_spoofedServiceKey),
    ...
    test('session bookmark targets are distinct and namespaced', () async {
      ...
      expect(
        session.capabilities<PluginBookmarkTargetStrategy>(),
        hasLength(2),
      );
      await session.close();

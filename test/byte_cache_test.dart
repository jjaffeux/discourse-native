...
void main() {
  group('URL admission and redirect handling', () {
  ...
  group('request lifecycle and work admission', () {
    ...
    test('bounds unique work waiting behind the semaphore', () async {
      ...
      expect(
        await cache.load('https://site.test/overflow'),
        orderedEquals([8]),
      );
  ...
  group('response bounds and deadlines', () {
    test(
    ...
    testWidgets('cancels a body whose headers arrive after the deadline', (
      tester,
    ) async {
      var cancelled = false;
      ...
      final load = cache.load('https://site.test/slow');
  ...
  group('memory retention and shared coordination', () {
    test(
    ...
    test(
      ...
      () async {
        ...
        final third = coordinator.acquire(origin.resolve('/third')).then((
          lease,
        ) {
          thirdGranted = true;
          return lease;
  ...
  group('persistent cache policy and lifecycle', () {
    ...
    test(
      'a no-store redirect keeps public final bytes process-local',
      () async {
    ...
    test(
      'rejects non-shareable directives and subtracts Age from s-maxage',
      () {
        final now = DateTime.utc(2026, 8, 11, 12);
        final forbidden = <Map<String, String>>[
          {
            'Cache-Control':
                'public, no-cache="etag, last-modified", max-age=3600',
          },
          {
            'cache-control':
    ...
    test(
      'pruning removes stale temporary files but not a live writer',
      () async {
        final directory = await Directory.systemTemp.createTemp(
          'discourse-native-byte-cache-',
        );
    ...
    test(
      'rejects an oversized disk entry before returning its bytes',
      () async {
        final directory = await Directory.systemTemp.createTemp(
          'discourse-native-byte-cache-',
        );
    ...
    test('reuses fresh immutable bytes across cache instances', () async {
      ...
      expect(
        requests,
        1,
        reason: 'the second cache represents a warm relaunch',
      );
    });

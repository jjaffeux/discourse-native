...
void main() {
  ...
  setUp(() async {
    directory = await Directory.systemTemp.createTemp('voice-report-exporter-');
  });
  ...
  test('a completed save atomically replaces the selected file', () async {
    ...
    expect(outcome, VoiceReportExportOutcome.saved);
  ...
  test('overlapping shares own distinct temporary files', () async {
    ...
    expect(await first, VoiceReportExportOutcome.shared);
    expect(await firstFile.exists(), isFalse);
    expect(await secondFile.exists(), isTrue);

    environment.release['second report']!.complete();
    expect(await second, VoiceReportExportOutcome.shared);
    expect(await secondFile.exists(), isFalse);
  });
}

const _expectedFilename = 'voice-diagnostics-2026-08-24T10-11-12-123456Z.jsonl';
...
final class _FakeExportEnvironment implements VoiceReportExportEnvironment {
...
final class _GatedShareEnvironment implements VoiceReportExportEnvironment {
  ...
  Future<VoiceReportExportOutcome> shareReport(
    ...
    await gate.future;
    return VoiceReportExportOutcome.shared;
  }

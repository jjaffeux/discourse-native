...
void main() {
  ...
  test(
    'is default-off, forwards structured logs, and gates raw records',
    () async {
  ...
  test(
    'selectively redacts secrets without erasing negotiation context',
    () async {
  ...
  test('recovers a corrupt tail and marks an interrupted capture', () async {
  ...
  test(
    'rotates bounded owner-private JSONL segments and keeps newest events',
    () async {
      final directory = await Directory.systemTemp.createTemp(
  ...
  test(
    'promotes interrupted compaction temps and deduplicates mixed segments',
    () async {
      final directory = await Directory.systemTemp.createTemp(
  ...
  test(
    'loads only a bounded decoded tail while preserving full JSONL',
    () async {
      final directory = await Directory.systemTemp.createTemp(
  ...
  test(
    'near-cap file history keeps only a byte-bounded decoded tail',
    () async {
      final directory = await Directory.systemTemp.createTemp(
  ...
  test('finds retained deep event IDs outside the decoded tail', () async {
    ...
    final controller = await ResenhaDiagnosticsController.create(
      persistence: FileResenhaDiagnosticsPersistence(file, decodedTailLimit: 1),
      clock: () => now,
    );
    addTearDown(controller.close);
  ...
  test(
    'multi-writer appends clamp timestamps and reject expired input',
    () async {
      final directory = await Directory.systemTemp.createTemp(
  ...
  test(
    'one writer stop does not clear another writer active capture',
    () async {
      final directory = await Directory.systemTemp.createTemp(
  ...
  test(
    'opening a second controller does not interrupt a live writer',
    () async {
      final directory = await Directory.systemTemp.createTemp(
      ...
      expect(_reportEventNames(report), isNot(contains('capture.interrupted')));
    },
  );

  test('compaction recovery never exposes mixed generations', () async {
  ...
  test(
    'SDK bridge is installed only for the explicit capture window',
    () async {
  ...
  test('capture enabled listenable ignores verbose record updates', () async {
    ...
    addTearDown(controller.close);
    var notifications = 0;
    controller.captureEnabledListenable.addListener(() => notifications += 1);
  ...
  test('coalesces burst persistence and UI-tail notifications', () async {
    ...
    for (var index = 0; index < 20; index += 1) {
      controller.recordRaw('sdk.spaced.$index');
  ...
  test(
    'bounds the decoded controller tail by bytes as well as count',
    () async {
  ...
  test('pending backpressure preserves the capture stop marker', () async {
  ...
  test('clear failure is surfaced and keeps retained state intact', () async {
    ...
    expect(await controller.buildJsonReport(), contains('before.failed.clear'));
  });

  test('clear resumes an old active capture with a fresh marker', () async {
  ...
  test('create failure never forwards original detail to General', () async {
  ...
  test('load failure never forwards original detail to General', () async {
  ...
  test('load failure keeps the durable store available for clear', () async {
  ...
  test('append failure never forwards original detail to General', () async {
  ...
  test('bridge detail stays deep-capture-only', () async {
  ...
  test('serializes close behind a pending SDK bridge install', () async {
  ...
  test('serializes sequential lifecycle calls and idempotent close', () async {
    final controller = await ResenhaDiagnosticsController.create(
      persistence: MemoryResenhaDiagnosticsPersistence(),
    );

    await controller.startCapture().timeout(const Duration(seconds: 1));
    await controller.stopCapture().timeout(const Duration(seconds: 1));
    await controller.close().timeout(const Duration(seconds: 1));
    await controller.close().timeout(const Duration(seconds: 1));
  });

  test('unwinds a bridge which throws after a partial install', () async {
  ...
  test('clamps backward clock jumps and expires only an old prefix', () async {
  ...
  test('expires seven-day history when an idle capture is exported', () async {
  ...
  test('passively expires idle retained history after seven days', () async {
  ...
  test(
    'events is a bounded tail while JSONL report keeps retained history',
    () async {

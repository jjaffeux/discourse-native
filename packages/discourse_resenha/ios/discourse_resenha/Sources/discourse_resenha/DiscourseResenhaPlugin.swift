...
/// The sole iOS registration point for Resenha.
///
/// Core runners know nothing about this channel. Flutter invokes this class
/// only when `discourse_resenha` is present in the application's package
/// graph, and the registrar retains the instance for the engine lifetime.
public final class DiscourseResenhaPlugin: NSObject, FlutterPlugin {
  private let coordinator: ResenhaCallKitCoordinator

  public static func register(with registrar: FlutterPluginRegistrar) {
    let plugin = DiscourseResenhaPlugin(messenger: registrar.messenger())
    registrar.publish(plugin)
  }

  private init(messenger: FlutterBinaryMessenger) {
    coordinator = ResenhaCallKitCoordinator(messenger: messenger)
    super.init()
  }
}
